...
namespace Snappy.Sharp
{
    ...
    {
        private const int MaxIncrementCopyOverflow = 10;
        ...
        {
            var nextValue = data[offset] & (BitMask - 1);
        ...
        public byte[] Decompress(byte[] compressed, int compressedOffset, int compressedSize)
        {
            ...
            Decompress(compressed, sizeHeader[1], compressedSize + compressedOffset - sizeHeader[1], data, 0,
                data.Length);

            return data;
        }

        public int Decompress(byte[] input, int inputOffset, int inputSize, byte[] output, int outputOffset,
            int outputLimit)
        {
            ...
            {
                var opCode = input[ipIndex++];
                var entry = OpLookupTable[opCode];
                var trailerBytes = (byte) (entry >> 11);

                var trailer = BitConverter.ToUInt32(input, ipIndex) & Wordmask[trailerBytes];
                ...
                {
                    var literalLength = (int) (length + trailer);
         ...
         */
        private static int[] DecompressTagSlow(byte[] input, int ipIndex, byte[] output, int outputLimit,
            int outputOffset, int opIndex)
        {
            // read the op code
            var opCode = input[ipIndex++];
            var entry = OpLookupTable[opCode];
        ...
        private static void CopyLiteral(byte[] input, int ipIndex, byte[] output, int opIndex, int length)
        {
            ...
            {
                var fastLength = (int) (length & 0xFFFFFFF8);
                if (fastLength <= 64)
                {
                    // copy long-by-long
        ...
        private static readonly ushort[] OpLookupTable =
        {
